...
class TestConstructor:
    ...
    def test_name_and_dim_match_settings(self) -> None:
        provider = LiteLLMEmbeddingProvider(
            _settings(dim=1024), embedding_fn=_StubEmbed(dim=1024)
        )
...
class TestEmbedBatch:
    ...
    def test_batches_respect_batch_size(self) -> None:
        stub = _StubEmbed(dim=4)
        provider = LiteLLMEmbeddingProvider(
            _settings(dim=4, batch_size=2), embedding_fn=stub
        )
...
class TestAdaptiveHalving:
    def test_halves_until_batch_fits(self) -> None:
        ...
        provider = LiteLLMEmbeddingProvider(
            _settings(dim=4, batch_size=8), embedding_fn=stub
        )
    ...
    def test_single_oversized_input_reraises(self) -> None:
        # max_batch=0 means even a single-element call 413s. The provider
        # cannot recover and must surface the original error.
        stub = _SizeCappedStub(max_batch=0, dim=4)
        provider = LiteLLMEmbeddingProvider(
            _settings(dim=4, batch_size=4), embedding_fn=stub
        )
    ...
    def test_non_413_errors_are_not_halved(self) -> None:
        ...
        provider = LiteLLMEmbeddingProvider(
            _settings(dim=4, batch_size=4), embedding_fn=_auth_fail
        )
